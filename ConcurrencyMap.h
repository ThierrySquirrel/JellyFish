...
namespace JellyFish {
	...
	public:bool isFind(K& key) {
		...
		typename std::map<K, V>::const_iterator mapFind;
		typename std::map<K, V>::const_iterator mapEnd;
	...
	public:std::map<K, V> getAll() {
		std::map<K, V> all;
		int forSize = this->locksOffset;

		for (int i = 0; i < forSize; i++) {
			locksContainer[i].lockShared();
			std::map<K, V> thisMap = containerAll[i];
			locksContainer[i].unlockShared();
