...
namespace JellyFish {
	...
	public:ConcurrencyDeque<V>(int& locksOffset) {
		this->init(locksOffset);
	}
	...
	public:std::deque<V> popBackAll() {
		std::deque<V> all;
		int size = this->locksOffset;

		for (int i = 0; i < size; i++) {
			locksContainer[i].lock();
			std::deque<V> thisVector = containerAll[i];
